...
class LangVocabulary(Vocabulary):
    def __init__(self, tokens: List[str]):
        super().__init__()
        self.specials = [UNK_TOKEN]
        self.DEFAULT_UNK_ID = lambda: 0
        self.stoi = defaultdict(self.DEFAULT_UNK_ID)
        self._from_list(tokens)

    def array_to_langs(self, array: List[int]) -> List[str]:
        langs = []
        for i in array:
            langs.append(self.itos[int(i)])
        return langs
    ...
    if vocab_file is not None:
        ...
        for i in dataset.examples:
            if field == "gls":
                ...
                tokens.extend(i.lang)
            elif field == "sign_lang":
                tokens.extend(i.sign_lang)
            else:
        ...
        elif field == "lang" or field == "sign_lang":
            vocab = LangVocabulary(tokens=vocab_tokens)
        else:
            raise ValueError("Unknown vocabulary type")
