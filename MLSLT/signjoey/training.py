...
from signjoey.helpers import save_dir_to_zip
...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.log_langs = train_config.get("log_langs", False)
            ...
            for batch in iter(train_iter):
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    if self.do_translation:
                        ...
                        # if self.log_langs:
                        #     # log langs scores
                        #     langs_valid_scores = val_res["langs_valid_scores"]
                        #     for lang, value in sorted(
                        #         langs_valid_scores.items(), key=lambda x: x[0]
                        #     ):
                        #         self.tb_writer.add_scalar(
                        #             f"{lang}/chrf", value["chrf"], self.steps
                        #         )
                        #         self.tb_writer.add_scalar(
                        #             f"{lang}/rouge", value["rouge"], self.steps
                        #         )
                        #         self.tb_writer.add_scalar(
                        #             f"{lang}/bleu", value["bleu"], self.steps
                        #         )
                        #         self.tb_writer.add_scalars(
                        #             f"{lang}/bleu_scores",
                        #             value["bleu_scores"],
...
def train(cfg_file: str) -> None:
    ...
    # store the code to the model dir
    code_dir = Path(__file__).parent
    save_dir_to_zip(code_dir, Path(trainer.model_dir) / "code.zip")
