...
def validate_on_data(
    ...
    with torch.no_grad():
        ...
        for valid_batch in iter(valid_iter):
            ...
            if do_recognition:
                ...
                # )
                all_gls_outputs.extend(batch_gls_predictions)
            if do_translation:
            ...
            # )
            all_attention_scores.extend(
                batch_attention_scores if batch_attention_scores is not None else []
            )
        ...
        if do_translation:
            ...
            txt_ref = ["".join(t).replace("▁", " ") for t in data.txt]
            txt_hyp = ["".join(t).replace("▁", " ") for t in decoded_txt]
            # post-process
            if level == "bpe":
                txt_ref = [bpe_postprocess(v) for v in txt_ref]
                txt_hyp = [bpe_postprocess(v) for v in txt_hyp]
            assert len(txt_ref) == len(txt_hyp)
            origin_txt_ref = txt_ref
            origin_txt_hyp = txt_hyp
            langs = [i[0] + "-" + j[0] for i, j in zip(data.sign_lang, data.lang)]
            sign_lang_kind = [
                i + "-" + j
                for i in model.sign_lang_vocab.itos[1:]
                for j in model.lang_vocab.itos[1:]
            ]
            lang_to_ref = dict(
                zip(
                    sign_lang_kind,
                    [[] for i in range(len(sign_lang_kind))],
                )
            )
            lang_to_hyp = dict(
                zip(
            ...
            for lang, ref, hyp in zip(langs, txt_ref, txt_hyp):
                lang_to_ref[lang].append(ref)
                lang_to_hyp[lang].append(hyp)
            langs_valid_scores = {}
            for lang in sign_lang_kind:
                txt_ref = lang_to_ref[lang]
                txt_hyp = lang_to_hyp[lang]
            ...
            txt_bleu = {
                "bleu1": np.mean(
                    [
                        value["bleu_scores"]["bleu1"]
                        for value in langs_valid_scores.values()
                    ]
                ),
                "bleu2": np.mean(
                    [
                        value["bleu_scores"]["bleu2"]
                        for value in langs_valid_scores.values()
                    ]
                ),
                "bleu3": np.mean(
                    [
                        value["bleu_scores"]["bleu3"]
                        for value in langs_valid_scores.values()
                    ]
                ),
                "bleu4": np.mean(
                    [
                        value["bleu_scores"]["bleu4"]
                        for value in langs_valid_scores.values()
                    ]
                ),
            }
            txt_chrf = np.mean([value["chrf"] for value in langs_valid_scores.values()])
            txt_rouge = np.mean(
                [value["rouge"] for value in langs_valid_scores.values()]
            )
    ...
    if do_translation:
        ...
        results["txt_ref"] = origin_txt_ref
        results["txt_hyp"] = origin_txt_hyp
        results["langs_valid_scores"] = langs_valid_scores
...
def test(
    ...
    model = build_model(
        ...
        sign_lang_vocab=sign_lang_vocab,
        gls_vocab=gls_vocab,
        txt_vocab=txt_vocab,
    ...
    logger.info("*" * 60)
    langs_valid_scores = dev_best_translation_result["langs_valid_scores"]
    for lang, value in sorted(langs_valid_scores.items(), key=lambda x: x[0]):
        logger.info(
            "[DEV] %s partition [Recognition & Translation] results:\n\t"
            "Best CTC Decode Beam Size: %d\n\t"
            "Best Translation Beam Size: %d and Alpha: %d\n\t"
            "WER %3.2f\t(DEL: %3.2f,\tINS: %3.2f,\tSUB: %3.2f)\n\t"
            "BLEU-4 %.2f\t(BLEU-1: %.2f,\tBLEU-2: %.2f,\tBLEU-3: %.2f,\tBLEU-4: %.2f)\n\t"
            ...
            value["bleu"] if do_translation else -1,
            value["bleu_scores"]["bleu1"] if do_translation else -1,
            value["bleu_scores"]["bleu2"] if do_translation else -1,
            value["bleu_scores"]["bleu3"] if do_translation else -1,
            value["bleu_scores"]["bleu4"] if do_translation else -1,
            value["chrf"] if do_translation else -1,
            value["rouge"] if do_translation else -1,
        )
        logger.info("*" * 60)
    ...
    langs_valid_scores = test_best_result["langs_valid_scores"]
    for lang, value in sorted(langs_valid_scores.items(), key=lambda x: x[0]):
        logger.info(
            "[TEST] %s partition [Recognition & Translation] results:\n\t"
            "Best CTC Decode Beam Size: %d\n\t"
            "Best Translation Beam Size: %d and Alpha: %d\n\t"
            "WER %3.2f\t(DEL: %3.2f,\tINS: %3.2f,\tSUB: %3.2f)\n\t"
            "BLEU-4 %.2f\t(BLEU-1: %.2f,\tBLEU-2: %.2f,\tBLEU-3: %.2f,\tBLEU-4: %.2f)\n\t"
