...
sp = spm.SentencePieceProcessor(
    model_file=str(
        Path(
            "~/data/pretrain-model/BPE/multi/1000000/multi.wiki.bpe.vs1000000.model"
        ).expanduser()
    )
)
...
def load_data(data_cfg: dict) -> (Dataset, Dataset, Dataset, Vocabulary, Vocabulary):
    ...
    def sp_tokenize_text(text):
        out = sp.encode(text, out_type=str)
        return out
    ...
    lang_field = data.Field(batch_first=True, tokenize=None)
    sign_lang_field = data.Field(batch_first=True, tokenize=None)
    ...
    lang_vocab = build_vocab(
        field="lang",
        min_freq=1,
        max_size=sys.maxsize,
        dataset=train_data,
    )
    sign_lang_vocab = build_vocab(
        field="sign_lang",
        min_freq=1,
        max_size=sys.maxsize,
        dataset=train_data,
    ...
    lang_field.vocab = lang_vocab
    sign_lang_field.vocab = sign_lang_vocab
