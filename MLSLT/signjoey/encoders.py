...
class TransformerEncoder(Encoder):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.logits = nn.ParameterList(
            [nn.Parameter(0.5 * torch.ones(num_layers, 2)) for _ in range(lang_num)]
        )
    ...
    def forward(
        ...
        for layer_index, layer in enumerate(self.layers):
            h = layer(x, mask, lang)
            lang_policy = []
            for lang_index in lang:
                lang_index = int(lang_index.item())
                lang_policy.append(policy[lang_index][layer_index])
            lang_policy = torch.stack(lang_policy)
            x = x * lang_policy[:, 1].reshape(-1, 1, 1) + h * lang_policy[:, 0].reshape(
                -1, 1, 1
            )
