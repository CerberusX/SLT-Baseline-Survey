...
from signjoey.loss import get_o_loss
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        lang_vocab: LangVocabulary,
        sign_lang_vocab: LangVocabulary,
        ...
        #     len(lang_vocab.itos), sgn_embed.embedding_dim, padding_idx=0
        # )
        self.sign_lang_embed = nn.Embedding(
            len(sign_lang_vocab.itos), sgn_embed.embedding_dim, padding_idx=0
        )

        self.lang_vocab = lang_vocab
        self.sign_lang_vocab = sign_lang_vocab
    ...
    def forward(
        ...
        encoder_output, encoder_hidden = self.encode(
            ...
            lang=sign_lang,
            lang_embedding=self.sign_lang_embed,
        )
    ...
    def encode(
        ...
        """
        input_sgn_length = sgn_length + 1
        # embed_lang = self.lang_embed(lang).unsqueeze(1)
        embed_lang = lang_embedding(lang).unsqueeze(1)
        embed_sgn = self.sgn_embed(x=sgn, mask=sgn_mask)
        embed_sgn = torch.cat([embed_lang, embed_sgn], 1)
        add_mask = sgn_mask.new_ones((sgn_mask.shape[0], 1, 1), dtype=torch.bool)
        input_sgn_mask = torch.cat([add_mask, sgn_mask], 2)

        return self.encoder(
            embed_src=embed_sgn,
            src_length=input_sgn_length,
            mask=input_sgn_mask,
            lang=lang,
        )
    ...
    def get_loss_for_batch(
        ...
        o_loss = get_o_loss(self)
        translation_loss = translation_loss + 0.1 * o_loss
    ...
    def run_batch(
        ...
        if self.do_translation:
            add_mask = batch.sgn_mask.new_ones(
                (batch.sgn_mask.shape[0], 1, 1), dtype=torch.bool
            )
            input_sgn_mask = torch.cat([add_mask, batch.sgn_mask], 2)
            # greedy decoding
...
def build_model(
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        ...
        encoder = TransformerEncoder(
            **cfg["encoder"],
            ...
            lang_num=len(sign_lang_vocab.itos) - len(sign_lang_vocab.specials),
        )
    else:
        ...
        if cfg["decoder"].get("type", "recurrent") == "transformer":
            decoder = TransformerDecoder(
                **cfg["decoder"],
                ...
                lang_num=len(lang_vocab.itos) - len(lang_vocab.specials),
            )
        else:
    ...
    word_embeding_path = cfg.get("word_embeding", "")
    if word_embeding_path:
        word_embeding_path = Path(word_embeding_path).expanduser()
        pretrain_vector = Vectors(
            name=str(word_embeding_path.name),
            cache=str(word_embeding_path.parent),
        )
